...
from zufall.lib.objekte.vektor import Vektor, _ZeilenVektor
from zufall.lib.funktionen.funktionen import is_zahl
from zufall.lib.objekte.ausnahmen import *
...
================================
zufall hat keine eigene Matrix-Klasse, es wird die MutableDenseMatrix-Klasse
von SymPy verwendet; somit sind alle Eigenschaften und Methoden für diese  
Matrizen verfügbar (siehe SymPy-Dokumentation)

Zur Erzeugung einer Matrix dient die zufall-Funktion 'Matrix', die neben der in
SymPy verwendeten Art auch die Erzeugung über die (Spalten-) Vektoren gestattet

Aus Gründen der Kompatibilität mit den anderen zufall-Objekten wurden einige 
wenige Eigenschaften/Methoden ergänzt und andere deutsch benannt

Die Multiplikation einer Matrix mit einem Vektor wird mittels Überladen der
__mul__-Methode realisiert, die Verkettung einer Matrix mit einem Vektor (Ope-
rator '|') mittels  einer dazu definierten __or__-Methode 

Die Anbindung der überladenen und eigenen Methoden an die Matrix-Objekte erfolgt
mittels der Funktion '_einrichten_matrix', die in anderen Moduln aufgerufen wer-
den kann 

"""


# Erzeuger-Funktion für Matrix in zufall
# --------------------------------------

def Matrix(*args, **kwargs):  
    """Funktion zur Erzeugung einer Matrix; neben der in SymPy üblichen Art 
        können auch die Spaltenvektorn einer Matrix angegeben werden, um diese
		zu erzeugen"""
    ...
    try:		
        if not args:
            raise ZufallError('mindestens zwei Vektoren angeben')		
        if iterable(args[0]):
            vektoren = args[0]
        ...
        if not all(isinstance(v, Vektor) for v in vektoren):
            raise ZufallError('Vektoren angeben')
...
def _char_poly(self):   
    ...
    if self.cols != self.rows:
        print('zufall: keine quadratische Matrix')
        return
    la = Symbol('lamda')
...
def _eig_wert(self):
    ...
    if self.rows in (2, 3) and self.cols == self.rows:
        ...
        if l > 250:    # Verhinderung der komplexen Ausgabe
            ...
            print('zufall: Übergang zur nummerischen Berechnung wegen komplexer Ausgabe')					  
            from sympy import N
            ew = [N(el, 12) for el in ew]	
        return ew			
    print('zufall: keine 2x2- bzw. 3x3-Matrix')
...
def _inverse(self):
    ...
    if self.cols != self.rows:
        ...
        return 
    if self.det() == 0:
        print('zufall: die Inverse existiert nicht (die Determinante ist = 0)')
        return
    return self.inv()
...
def _sch_el(self, *wert, **kwargs):
    ...
    if not schar or len(self.sch_par) > 1:
        print('zufall: keine Schar mit einem Parameter')	
        return		
    if not wert or len(wert) != 1:
        print('zufall: einen Wert für den Scharparameter angeben') 
        return	
    p = Tuple(*self.sch_par)[0]
    wert = sympify(*wert)
    if not is_zahl(wert):
        print('zufall: für den Scharparameter Zahl oder freien Parameter angeben')	
        return		
    try:
        wert = nsimplify(wert)		
    except RecursionError:
...
def _einrichten_matrix():
    ...
    def _mul(self, other):  
        ...
        if self.anzSpalt != other.dim:
            print('zufall: die Dimensionen sind unverträglich, Multiplikation nicht möglich')
            return
    ...
    def _or(self, other):
        ...
        if isinstance(other, Vektor) and other.dim == self.dim[0]:
            ...
            print('zufall: Vektor mit passender Dimension angeben')
            return 			
        vekt.append(other)
        return Matrix(*vekt)
...
def matrix2vektor(m):
    ...
    if not isinstance(m, SympyMatrix):
        print('zufall: Matrix angeben')
        return
    if m.shape[1] != 1:
        print('zufall: die Spaltenanzahl ist > 1, Konvertierung nicht möglich')
        return		
    ve = m.vekt
    return Vektor(*ve)
...
def matrix_hilfe(h):
    ...
    if h == 3:
        ...
        print("m.eig_wert      m.eigenvects()   Eigenvwerte             (**)") 
